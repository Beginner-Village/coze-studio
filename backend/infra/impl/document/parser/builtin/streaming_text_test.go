...
func TestStreamingTextChunker_ShortText(t *testing.T) {
	...
	docs, err := chunker.Chunk(context.Background(), strings.NewReader("hello world"))
...
func TestStreamingTextChunker_LongText(t *testing.T) {
	...
	docs, err := chunker.Chunk(context.Background(), strings.NewReader(input))
	require.NoError(t, err)
...
func TestStreamingTextChunker_Empty(t *testing.T) {
	chunker := &streamingTextChunker{chunkSize: 100, overlap: 0}
	docs, err := chunker.Chunk(context.Background(), strings.NewReader(""))
	require.NoError(t, err)
	assert.Len(t, docs, 0)
}
