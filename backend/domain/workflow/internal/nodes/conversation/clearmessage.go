...
func (c *MessageClear) Clear(ctx context.Context, input map[string]any) (map[string]any, error) {
	...
	if agentID != nil {
		return nil, vo.WrapError(errno.ErrConversationNodesNotAvailable, fmt.Errorf("in the agent scenario, clear conversation history is not available"))
	}
	if appID == nil {
		return nil, vo.WrapError(errno.ErrConversationNodesNotAvailable, fmt.Errorf("clear conversation history node, app id is required"))
	}
