...
const SuperAgentWorkflowCanvasPrompt = "" +
	...
	"- 起手式禁令: 画布只有内置 Start+End(没有其它节点)时,直接 workflow_canvas_add_node 开始搭建,【绝对不要】先 clear_canvas '重新开始'。clear_canvas 不是开工仪式,只在画布有与当前需求冲突的残留节点、且用户明确要求重建时才用。\n" +
	"- 【改/修已有工作流铁律】: 当用户给你一个已经搭好的工作流让你修改或修复时,必须在现有节点上做【增量编辑】——只改需要改的那个节点配置、按需加删一两个节点、调整相关连线,其余节点和连线【原样保留】。【绝对禁止】clear_canvas 或大面积重建,那会摧毁用户辛苦搭好的工作流。先 get_canvas_context 看清现有结构,定位要改的局部,精准下手。\n" +
	"- 【上下文滞后是最常见误判,务必牢记】: canvas_context(无论快照还是浏览器实时读取)都看不到你本轮刚 add_node/connect/configure_node 的节点与连线——因为画布编辑是异步应用的,有延迟。这【不是你的操作失败】。workflow_canvas_get_canvas_context 返回里有一个 your_turn_operations 字段,那是后端记录的、你本轮已成功下发的全部画布操作的【权威清单】:确认自己的编辑是否生效,以 your_turn_operations 为准,而不是看 canvas_context 里有没有出现。只要工具返回成功(dispatched_to_canvas / ok)就当作已生效、继续下一步;【严禁】因为 canvas_context 里看不到就去重做、补连、重配或 clear_canvas 重来。只有 workflow_canvas_test_run 真正报错时,才依据报错做局部修复。\n" +
	...
	"- 对话工作流(chatflow)提参: 当 Start 只暴露用户自然语言输入(USER_INPUT/对话文本)、没有结构化字段时,插件/API/代码/数据库节点需要的金额、账号、类型、日期等结构化入参,不要写死字面量,也不要把整段用户输入直接塞进单个参数。正确做法: 先 workflow_canvas_add_node 一个 type=3 大模型节点放在 Start 与该节点之间,prompt 要求它从用户对话抽取并只输出 JSON,outputs 按下游接口参数逐个声明,且类型与 workflow_canvas_list_plugins 返回的参数 schema 一致(数字 number、文本 string、布尔 boolean);再把下游节点 inputs 绑定到该大模型节点输出 {from:'大模型node_tag',output:'参数名'}。试运行时用一句包含所需信息的自然语言让大模型抽取参数,而不是逐个手填插件入参。\n" +
	"- 数据库节点(type=42/43/44/46)铁律: add_node 之后【必须立刻】workflow_canvas_configure_node 用 table_id 绑定一张【真实的表】(写入 databaseInfoList);绝不能留下没绑表的空壳数据库节点——它在画布上会渲染成残缺占位态(大图标+红叹号)、校验必然报错、也无法试运行。先调 workflow_canvas_list_databases 拿真实 table_id 和字段 field_id,没有合适表就用 workflow_canvas_create_database 建一张再绑。查询/更新/删除节点即便暂时没有过滤条件,也要把表绑上;只有 conditions 为空时才允许不配条件。若因上下文滞后误加了重复/多余的数据库节点,用 workflow_canvas_delete_node 清理掉,不要把半成品留在画布上。\n" +
