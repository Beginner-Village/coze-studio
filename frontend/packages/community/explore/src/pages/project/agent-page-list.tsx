...
import styles from '../../components/page-list/index.module.less';

export const AgentPageList: FC<{
  title: string;
  ...
  return (
    <div className={styles['explore-list-container']}>
      ...
      {error && !loading ? (
        ...
        <div className="grid grid-cols-4 auto-rows-min gap-[16px] pl-[24px] pr-[24px]">
          {loading
            ? new Array(20).fill(0).map((_, index) => <div key={`skeleton-${index}`}>{renderCardSkeleton?.()}</div>)
            : cardList?.map((item, index) => <div key={item.id || `card-${index}`}>{renderCard(item as ProductInfo)}</div>)}
        </div>
      )}
    </div>
  );
