...
export const QALocal = (props: ImportKnowledgeMenuSourceModuleProps) => {
  const { onClick } = props;
  return (
    <KnowledgeSourceMenuItem
      title={I18n.t('knowledge_qa_local_title')}
      icon={<IconCozDocument className="w-4 h-4" />}
      testId={`${KnowledgeE2e.SegmentDetailDropdownItem}.${UnitType.QA_DOC}`}
      value={UnitType.QA_DOC}
      onClick={() => onClick(UnitType.QA_DOC)}
    />
  );
};

export const QALocalModule: ImportKnowledgeMenuSourceModule = {
  Component: QALocal,
};
