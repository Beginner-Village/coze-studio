...
export async function getTracesMetaInfo(
  req: GetTracesMetaInfoRequest,
): Promise<GetTracesMetaInfoResponse> {
  const qs = toQueryString(req as Record<string, unknown>);
  return request<GetTracesMetaInfoResponse>(`${BASE}/traces/meta_info${qs}`, {
    method: 'GET',
  });
}
